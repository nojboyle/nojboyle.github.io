...
function draw() {
	let flying1 = new FlyingBox(-10,5,0);
	container.spinY(1);
	/*flying1.fly();*/
	fill(random(0,255), random(0,255), random(0,255));
	circle(random(width), random(height), random(5,30));
	let cchange = new ColorChanging(-20,5,0);
...
class ColorChanging{
	...
	constructor(x,y,z){
		...
		world.add(this.colorSphere);
	}
	changecolor() {
		this.colorSphere.setColor(random(0,255),random(0,255),random(0,255));
	}
}
...
class FlyingBox{
	...
	constructor(x,y,z){
		...
		this.boxFlying = new Sphere({
			x:x, y:y, z:z,
			scaleX:2,
			scaleY:2,
			scaleZ:2,
		});
		world.add(this.boxFlying);
	...
	fly() {
		...
		this.boxFlying.setPosition(xMovement, yMovement, zMovement);
	}
}